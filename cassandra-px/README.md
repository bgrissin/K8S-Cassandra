Within this lab, all storage volumes are dynamically created and managed by PX.    The cassandra stateful PODs in this lab are also represented via a K8S headless service called cassandra.  The major difference this lab has from the local volume lab are that storage volumes being used by the statefulset cassandra PODs are dynamically created and managed in line by PX as defined by persistent volumes and claims needed to fulfill the storage requirements for each scheduled statefulset cassandra POD.    

The automation possibilities available using PX with statefulsets along with K8S persistent claims and volumes lends well to many devops stories when trying to associate and align stateless portions of applications stacks to stateful layers.   As noted during the previous local storage lab, volumes were manually configured and presented on all nodes within the cluster in advance of any scheduled POD startups or scale/failover of cassandra services.  Performing volume configurations up front and out of band of the statefulset POD and defined volumes within becomes very reactive and often requires maintenance windows to accomplish configuration changes that may exist or become operationally necessary over time.    Using dynamic provisioning via a thin storage management layer provided by PX, most changes or fixes to statefulset volumes and storage configurations can be acheived in line without having to tear down and redeploy complete stacks to get changes implemented.   
...
Notice the directory and file structure differs a bit from the local lab structure. The StorageClass directory and files contain additional configurations used for the installation of PX (px-spec.yaml) and for persistent volumes and volume definition claims used by in the cassandra statefulset configurations.  Also if you look at the cassandra-statefulset.yaml storage configurations, you will notice differences in the storage configuration sections.  Also the start-cassandra.sh script also starts the persistent volume services that resides in the StorageClass directory.  
...
PX prepares and presents all volumes to the cassandra statefulsets dynamically and are configured and aligned for consumption by the service definitions specified within the statefulset persistent claims and volumes.  In the previous local labs, all storage volumes had to be pre-confgured with fdisk, formatted with a useable file system, manually announce the new volume configs to each kernel (or reboot the host) and have new volumes mounted persistently prior to starting up, scaling or failing over any PODs.  PX provides serious automation capabilities for stateful container workloads that provides significant amounts of simplicity and value for DevOps efforts to automate thorughout entire application stacks.   If your into Devops and are working within a distributed containered environment, then you absolutely should be looking at the PX solution for automating the storage and volume requirements within your stacks.

The features and capabilities PX dynamic provisioning provides for a distributed container cluster also extremely enhances RTO and RPO stories for distributed application container stacks.  Dynamic provisioning with PX reduces the time and complexities that often increase and impeed successful RPO and RTO objectives that are required to achieve an enterprise production status.   
...
The logs of the cassandra container within cassandra-0 on cassandra1 node can be watched while the cassandra POD recovery is taking place. Within the logs, message references signaling to other cassandra members when nodes are down or up using internally managed SeedProvided IP addresses and log references of commit replays being performed or retried occur during the startup of a POD.  Eventually message entries will be displayed that inform the client endpoints are updated and ready.  Its important to note that when a cassandra POD reaches a client available and running state using the kubectl log and display outputs, it does not imply that recovery has fully occured, but only that the cassandra client has become available for accepting connections.  Even after the cassandra client via the kubectl POD status is announced as ready and available, there are commits and repair processes that are still taking place that need to occur before all cassandra rings should be considered complete replicas with all other cassandra rings and members within the cassandra cluster.  However, reaching a useable active client endpoint status in an automated and reasonably fast timeframe certainly improves RPO metrics, especially when multiple clients are necessary to keep bottlenecks at bay that might occur when too few clients are available to collect and commit data.

The amount of time it takes to get to a new cassandra member to a complete running replica state is another outcome that is available for review and area of possible improvement that PX can bring signicant improvements to your service levels.  These labs employ a Simplestrategy within a single data center, and a relatively small, static amount of data, therefore does not present well for being able to demonstrate and produce steps representive of large data set within a large production grade environment where bootstrapping improvement possibilities offered by using PX can be visibly shown.  However its important to introduce and discuss these potential enhancements as they most certainly should be considered for real world, production at scale environments.  

As mentioned in the opening introduction of this repo, Cassandra uses two stages for recovery from a failure.   The first stage of the cassandra recovery process is called bootstrapping and the second stage is called repair.   Bootstrapping is the portion of cassandra recovery that involves the restoration of committed data that already is stored within the cassandra rings and respective member replicas.  Repair is the followup process to bootstrapping that syncs any and all uncommited transactions that may have arrived while bootstrapping is being performed.   Full recovery times for a cassandra node is a combination of bootstrapping and repair processes together.   Acceptable recovery times can become a problem when the time to bootstrap takes too long to occur.  In fact its a fairly common occurence when using a networktopologystrategy for clusters with data are spread across multiple data centers that sync/copy times can be very latent for large data sets during bootstrapping copy operations.  PX provides a snap shot capability that can be deployed to occur across all nodes even in a mutidata center configuration.  Using recent and local snaps to perform full data set recovery can significantly speed up the bootstrap process by removing the need to copy data across nodes within a multi-region or multi data center cassandra cluster.  Unaddressed latency concerns during bootstrapping cassandra nodes will become even more noticeable and add further negative effects to RTO and RPO as your data sets expand to many petabytes of data that needs to occur during scale or failover events.   It seems as almost a requirement that using a snap solution like PX should be considered when using distributed multi-region data center scenarios  
