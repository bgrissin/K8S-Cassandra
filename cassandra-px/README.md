Within this lab, all storage volumes are dynamically created and managed by PX.    The cassandra stateful PODs in this lab are also represented via a K8S headless service called cassandra.  The major difference this lab has from the local volume lab are that storage volumes being used by the statefulset cassandra PODs are dynamically created and managed in line by PX and by using defined persistent volumes and claims to fulfill the storage requirements for each scheduled statefulset cassandra POD.  Specifically what this means is that POD volumes are dynamically created on demand and managed via definitions outlined within the cassandra services.   
