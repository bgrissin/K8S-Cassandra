Within this lab, all storage volumes are dynamically created and managed by PX.    The cassandra stateful PODs in this lab are also represented via a K8S headless service called cassandra.  The major difference with this lab from the local volume lab are that storage volumes being used by the statefulset cassandra PODs are dynamically created and managed in line by using persistent volumes and claims along with PX to fulfill the storage requirements for each scheduled statefulset cassandra POD.  Specifically what this means is that POD storage volumes are dynamically created and managed on demand by definitions outlined within the cassandra services.   The automation available using PX with statefulsets lends well to many devops stories when trying to achieve synergies for stateless portions of applications stacks to stateful layers.   As noted during the previous local storage lab, local storage was manually configured and presented on all nodes within the cluster in advance of any scheduled POD startups or scale/failover of cassandra services.  Performing storage configurations up front and out of band for statefulset POD volumes becomes very reactive and often requires maintenance windows to accomplish configuration changes that may become operationally necessary over time.    Using dynamic provisioning via a thin storage management layer provided by PX, most changes or fixes to statefulset volumes and storage configurations can be acheived in line without having to tear down and redeploy complete stacks to get changes implemented.   
