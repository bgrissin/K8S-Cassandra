...
As mentioned in the opening introductions of this repo, Cassandra provides a very robust set of processes and archtiecture that provides a  highly resilient and available experience.  This is accomplished by employing a multi-client, multi-endpoint capability and internal archtiectures that facilitate processes to fulfill full recovery of both commited and uncommited data to occur in a 2 stage process. The first stage of the recovery process is called bootstrapping and the second stage is called repair.   Bootstrapping is the portion of cassandra recovery that involves committed data restoration.   Repair is the followup to bootstrapping that syncs any and all uncommited transactions that occurred while bootstrap was running.   Full recovery times can become a burden when the time to bootstrap takes too long to occur, such as when using a networktopologystrategy where data is spread across multiple data centers and sync times can be very latent for large recovery copy operations.  PX provides snap shot capabilities that can be employed to occur across all nodes even in a mutidata center configuration.  Using snaps to get to full data set recovery significantly speeds up the bootstrap process by removing the need to copy data across nodes within a multi-region or multi data center cassandra cluster.  Unaddressed latency concerns will continue become even more noticeable and add further negative effects to RTO and RPO as your data sets expand to many petabytes of data that needs to occur during scale or failover events.  
